...
const UserAccountScreen = ({ navigation }: any) => {
    const { userName, userPhoto } = useAuth(); // Use the custom hook
    ...
    return (
        <View className='flex-1 bg-Black'>
            ...
            <View className='mx-space_16 mt-[70px]'>
                <AppHeader name="close" header="My Profile" action={() => { }} />
            </View>
            ...
            <View className='my-5 mx-5'>
                {ButtonText.map((button, index) => (
                    <View key={index} className='justify-between w-full mt-5'>
                        <ButtonProfile text={button.text} subtext={button.subtext} subtext_2={button.subtext_2} icon={button.icon} />
                    </View>
                ))}
            </View>
        </View>
